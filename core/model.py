...
class Linear(nn.Module):
    ...
    def __init__(self, in_dim:int, out_dim:int, device=None, dtype=None) -> None:
        ...
        self.weight = nn.Parameter(data=nn.init.trunc_normal_(torch.empty(out_dim, in_dim)).to(device=device, dtype=dtype), requires_grad=True)
...
class Embedding(nn.Module):
    ...
    def __init__(self, num_embeddings:int, embedding_dim:int, device=None, dtype=None) -> None:
        ...
        self.weight = nn.Parameter(data=nn.init.trunc_normal_(torch.empty(num_embeddings, embedding_dim)).to(device=device, dtype=dtype))
...
class RMSNorm(nn.Module):
    ...
    def __init__(self, d_model:int, eps:float=1e-5, device=None, dtype=None) -> None:
        ...
        self.device:torch.device|None = device
        self.dtype:torch.dtype|None = dtype
        self.weight = nn.Parameter(data=nn.init.trunc_normal_(torch.empty(d_model)).to(device=device, dtype=dtype))
...
class MHARope(nn.Module):
    ...
    def __init__(self, d_model:int, num_heads:int, rope:nn.Module, device="cpu")-> torch.Tensor:
...
class TransformerBlock(nn.Module):
    ...
    def __init__(self, d_model:int, num_heads:int, d_ff:int, rope:nn.Module, device="cpu") -> None:
        super().__init__()
        self.d_model = d_model
        self.num_heads = num_heads
        self.d_ff = d_ff
...
class TransformerLM(nn.Module):
    ...
    def __init__(self, vocab_size:int, context_length:int, d_model:int, num_layers:int, num_heads:int, d_ff:int, rope_theta:float, device="cpu") -> None:
        super().__init__()
...
class AdamW(torch.optim.Optimizer):
    ...
    def __init__(self, params: Iterable[Dict[str, Any]], lr: float, betas:tuple[float, float], eps:float, weight_decay:float) -> None:
    ...
    def step(self, closure: Optional[Callable] = None) -> None:
        ...
        for group in self.param_groups:
            ...
            for p in group["params"]:
                ...
                second_moment.mul_(beta2).addcmul_(gradients, gradients, value=(1-beta2))
...
def learning_rate_schedule(iter_num:int, lr_max:float, lr_min:float, warmup_iters:int, cosine_cycle_iters:int)-> float:
    ...
    if iter_num < warmup_iters:
        ...
        learning_rate = lr_min + (0.5 * (1 + math.cos((iter_num-warmup_iters)/(cosine_cycle_iters-warmup_iters) * math.pi))) * (lr_max-lr_min)
...
def gradient_clipping(parameters: Iterable[torch.nn.Parameter], max_l2_norm:float) -> Iterable[torch.nn.Parameter]:
    ...
    total_norm = math.sqrt(sum([torch.norm(input=param.grad, p=2).pow(2) for param in parameters if param.grad is not None]))
...
def get_batches(dataset: npt.NDArray, batch_size:int, context_length:int, device:str) -> tuple[torch.tensor, torch.tensor]:
...
def save_checkpoint(model:torch.nn.Module, optimizer:torch.optim.Optimizer, iteration:int, out, config={}):
