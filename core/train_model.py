...
from core.generate import generate_with_temperature
from core.model import AdamW, TransformerLM, calculate_cross_entropy, get_batches, gradient_clipping, learning_rate_schedule, load_checkpoint, save_checkpoint
...
def get_args():
    ...
    resume_group.add_argument('--train_resume', action="store_true", help="Resume training from previous run")
...
if __name__ == "__main__":
    ...
    # Conditional logic
    if args.train_resume and not args.wandb_run_id:
    ...
    model_run = f"{logger_run.project}-{logger_run.name}"
   ...
   # to check if checkpoint exists and run from there or error for no dir found nor model found 
    ...
    current_checkpoints = [f for f in model_checkpoint_path.rglob(f"*.pt") if f.is_file()]
    ...
    train_tokenized_dataset = MegatronDataset(str(train_dataset_bin_path), 
                                        str(train_dataset_idx_path))

    valid_tokenized_dataset = MegatronDataset(str(valid_dataset_bin_path), 
                                        str(valid_dataset_idx_path))

    model = TransformerLM(**model_config)
    model = torch.compile(model, backend="aot_eager")
    optimizer = AdamW(params=model.parameters(),lr=args.lr, betas=(args.beta1, args.beta2), eps=args.eps, weight_decay=args.weight_decay)
    step = 0
    
    if args.train_resume:
        step = load_checkpoint(src=str(model_checkpoint_path), model=model, optimizer=optimizer)
        print(f"Resuming training from step: {step} \nmodel config: {model_config} \nHyperParameters: {hyperparameters}")
    else:
        print(f"Training start with \nmodel config: {model_config} \nHyperParameters: {hyperparameters}")

    while step < args.num_steps:

        # (x , y) = get_batches(tokenized_data, 32, args.max_seq_len, "cpu")
        (x, y) = train_tokenized_dataset.get_batch(32, args.max_seq_len, "cpu")
        logits = model(x)
        ...
        optimizer.step()
        lr = learning_rate_schedule(step, lr_max=args.lr_max, lr_min=args.lr_min, warmup_iters=args.warmup_iters, cosine_cycle_iters=args.cosine_cycle_iters)
        ...
        if step % 100 == 0 and step >= 10:
            ...
            save_checkpoint(model, optimizer, step, out=str(model_checkpoint_path), config=model_config)
            ...
            (x_val, y_val) = valid_tokenized_dataset.get_batch(32, args.max_seq_len, "cpu")
